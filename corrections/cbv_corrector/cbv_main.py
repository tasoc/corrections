...
#		C = slin.lstsq(X, flux[idx])[0]
	...
	def _prior_load(self, cbv_area, data_path, ncbvs=3):
		...
		for jj, ncbv in enumerate(np.arange(1,ncbvs+1)):
			with open(os.path.join(data_path, 'Rbf_area%d_cbv%i.pkl' %(cbv_area,ncbv)), 'rb') as file:
				I = dill.load(file)
				P['cbv_area%d_cbv%i' %(cbv_area, ncbv)] = I
			with open(os.path.join(data_path, 'Rbf_area%d_cbv%i_std.pkl' %(cbv_area,ncbv)), 'rb') as file:
				Is = dill.load(file)	
